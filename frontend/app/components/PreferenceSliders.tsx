...
const SIGNALS = [
  { key: "w_quality",    label: "Quality",    color: "#38BDF8", plain: "How good/accurate the content is" },
  { key: "w_popularity", label: "Popularity", color: "#818CF8", plain: "How many people clicked or viewed it" },
...
function AnimatedBar({ width, color }: { width: number; color: string }) {
  ...
  return (
    <div style={{ background:"#1F2937", height:4, borderRadius:2, overflow:"hidden" }}>
      <div ref={ref} style={{ background:color, height:4, borderRadius:2, width:"0%", transition:"width 0.6s cubic-bezier(0.4,0,0.2,1)" }} />
    </div>
  );
}
...
function ScoreTooltip({ item }: { item: Item }) {
  return (
    <div style={{
      ...
      background:"#0F1923", border:"0.5px solid #38BDF8",
      ...
      {Object.entries(item.signal_breakdown || {}).map(([sig, val]) => (
        <div key={sig} style={{ marginBottom:6 }}>
          <div style={{ display:"flex", justifyContent:"space-between", marginBottom:3 }}>
            ...
            <span style={{ fontSize:10, fontWeight:700, color:SIG_COLORS[sig]||"#38BDF8" }}>{(val as number).toFixed(4)}</span>
          </div>
          <div style={{ background:"#1F2937", height:3, borderRadius:2 }}>
            <div style={{ background:SIG_COLORS[sig]||"#38BDF8", height:3, borderRadius:2, width:`${Math.min(((val as number)/item.composite_score)*100,100)}%` }} />
          </div>
        </div>
      ))}
      <div style={{ borderTop:"1px solid #1F2937", marginTop:8, paddingTop:6, display:"flex", justifyContent:"space-between" }}>
        <span style={{ fontSize:10, color:"#64748B" }}>Composite</span>
        <span style={{ fontSize:11, fontWeight:800, color:"#38BDF8" }}>{item.composite_score.toFixed(4)}</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: `linear-gradient(90deg, transparent, rgba(56,189,248,0.04) ${pos}%, rgba(56,189,248,0.08) ${pos + 20}%, transparent ${pos + 40}%)`,
      }} />
      <div style={{ padding: "10px 13px", display: "flex", alignItems: "center", gap: 12 }}>
        <div style={{ width: 36, display: "flex", flexDirection: "column", gap: 4, alignItems: "center" }}>
          <div style={{ width: 28, height: 14, borderRadius: 3, background: "#1F2937" }} />
          <div style={{ width: 16, height: 8, borderRadius: 2, background: "#1F2937" }} />
        </div>
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
          <div style={{ width: `${70 + (i * 7) % 25}%`, height: 10, borderRadius: 3, background: "#1F2937" }} />
          <div style={{ width: `${40 + (i * 11) % 20}%`, height: 8, borderRadius: 2, background: "#1F2937" }} />
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 4, alignItems: "flex-end" }}>
          <div style={{ width: 40, height: 12, borderRadius: 3, background: "#1F2937" }} />
          <div style={{ width: 24, height: 8, borderRadius: 2, background: "#1F2937" }} />
        </div>
        <div style={{ width: 50 }}>
          <div style={{ width: "100%", height: 4, borderRadius: 2, background: "#1F2937" }} />
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      <div style={{ background:"#0F1923", border:"0.5px solid #1F2937", borderRadius:8, padding:"14px 16px" }}>
        ...
        {SIGNALS.map(s => (
          <div key={s.key} style={{ marginBottom:16 }}>
            ...
            />
            <div style={{ height:2, background:"#1F2937", borderRadius:1, marginTop:4 }}>
        ...
        <div style={{ display:"flex", gap:8, marginTop:8 }}>
          <button onClick={() => onChange({ w_quality:parseFloat((Math.random()).toFixed(2)), w_popularity:parseFloat((Math.random()).toFixed(2)), w_recency:parseFloat((Math.random()).toFixed(2)), w_diversity:parseFloat((Math.random()).toFixed(2)), w_engagement:parseFloat((Math.random()).toFixed(2)) })}
            style={{ background:"rgba(56,189,248,0.1)", color:"#38BDF8", border:"0.5px solid rgba(56,189,248,0.4)", borderRadius:5, padding:"6px 14px", fontSize:11, fontWeight:600, cursor:"pointer", textTransform:"uppercase", letterSpacing:"0.04em" }}>
            Randomize
          </button>
          <button onClick={() => onChange({ w_quality:0.5, w_popularity:0.3, w_recency:0.2, w_diversity:0.2, w_engagement:0.4 })}
            style={{ background:"transparent", color:"#64748B", border:"0.5px solid #1F2937", borderRadius:5, padding:"6px 14px", fontSize:11, fontWeight:600, cursor:"pointer", textTransform:"uppercase", letterSpacing:"0.04em" }}>
            Reset
          </button>
        </div>
      </div>
      ...
      <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
        {CATS.map(c => (
          <button key={c} onClick={() => onCategoryChange(c)} style={{
            ...
            background: category===c ? "rgba(56,189,248,0.1)" : "transparent",
            color: category===c ? "#38BDF8" : "#64748B",
            border: `0.5px solid ${category===c ? "rgba(56,189,248,0.4)" : "#1F2937"}`,
      ...
      <div>
        <div style={{ fontSize:10, color:"#64748B", textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:10, display:"flex", alignItems:"center", gap:6 }}>
          {loading ? (
            <>
              <div style={{ width:8, height:8, borderRadius:"50%", border:"1.5px solid #1F2937", borderTop:"1.5px solid #38BDF8", animation:"spin 0.8s linear infinite", flexShrink:0 }} />
        ...
        {loading
          ...
          ? (
            <div style={{ background:"#0F1923", border:"0.5px solid #1F2937", borderRadius:8, padding:"32px 16px", textAlign:"center" }}>
            ...
            return (
              <div key={item.id}
                ...
                style={{
                  position:"relative",
                  background:"#0F1923",
                  border:`0.5px solid ${moved ? (d > 0 ? "rgba(52,211,153,0.25)" : "rgba(248,113,113,0.25)") : "#1F2937"}`,
                ...
                <div style={{ width:36, textAlign:"center", flexShrink:0 }}>
                  <div style={{ fontSize:16, fontWeight:800, color: i===0?"#38BDF8":i===1?"#818CF8":i===2?"#34D399":"#64748B" }}>
                    #{i+1}
                  </div>
                  {moved ? (
                    ...
                    <div style={{ fontSize:9, color:"#1F2937", lineHeight:1.2, marginTop:1 }}>—</div>
                  )}
                </div>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <div style={{ fontSize:10, color:"#64748B", marginTop:2 }}>
                    ...
                    Quality <span style={{ color:"#38BDF8" }}>{(item.quality_score*100).toFixed(0)}%</span>
                    <span style={{ margin:"0 5px" }}>·</span>
                ...
                <div style={{ textAlign:"right", flexShrink:0 }}>
                  <div style={{ fontSize:15, fontWeight:700, color:"#38BDF8", fontVariantNumeric:"tabular-nums" }}>
                ...
                <div style={{ width:50, flexShrink:0 }}>
                  <AnimatedBar
                    key={`${item.id}-${item.rank}-${item.composite_score}`}
                    width={item.composite_score * 100}
                    color={i===0?"#38BDF8":i<=2?"#818CF8":"#64748B"}
                  />
                </div>
              </div>
            );
