...
export default function OutcomesPanel({ outcomes }: Props) {
  ...
  const plotlyLayout = {
    ...
    hoverlabel: {
      bgcolor: "#0F1923", bordercolor: "#38BDF8",
  ...
  const catOption = {
    ...
    xAxis:{ type:"value", axisLabel:{ color:"#64748B", fontSize:9, formatter:(v:number)=>`${v.toFixed(1)}%` }, splitLine:{ lineStyle:{ color:"#1F2937" } } },
    yAxis:{ type:"category", data:catData.map(([k])=>k), axisLabel:{ color:"#64748B", fontSize:11 }, axisLine:{ lineStyle:{ color:"#1F2937" } } },
    tooltip:{ backgroundColor:"#0F1923", borderColor:"#1F2937", textStyle:{ color:"#E2E8F0", fontSize:11 }, formatter:(p:any)=>`${p.name}: <span style="color:#38BDF8;font-weight:700">${p.value.toFixed(1)}%</span>` },
    series:[{ type:"bar", barMaxWidth:20,
      data: catData.map(([cat,val]) => ({ value:parseFloat(val.toFixed(1)), itemStyle:{ color:CAT_COLOR[cat]||"#38BDF8", borderRadius:[0,3,3,0], opacity:0.85 } })),
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Card style={{ background:"#0F1923", border:"0.5px solid #1F2937" }}>
        <CardContent style={{ padding:"12px 16px" }}>
          <p style={{ fontSize:11, color:"#94A3B8", lineHeight:1.7, margin:0 }}>
            <span style={{ color:"#38BDF8", fontWeight:600 }}>The real question:</span> Does the algorithm surface the <em>best</em> content, or just the most popular? Each dot is one piece of content. If quality predicted exposure, dots would rise from bottom-left to top-right. If it's flat — quality doesn't matter.
          </p>
        </CardContent>
      </Card>

      {/* Correlation stat */}
      <Card style={{ background:"#0F1923", border:`0.5px solid ${corrColor}30` }}>
        <CardContent style={{ padding:"14px 16px", display:"grid", gridTemplateColumns:"auto 1fr", gap:20 }}>
          ...
          <div style={{ borderLeft:"1px solid #1F2937", paddingLeft:20 }}>
            ...
            <div style={{ fontSize:11, color:"#94A3B8", lineHeight:1.6 }}>
              A correlation near <span style={{ color:"#F87171" }}>0</span> means bad content gets as much airtime as good content. Go back to <span style={{ color:"#38BDF8" }}>Preference Sliders</span> and raise the <strong style={{ color:"#38BDF8" }}>Quality weight</strong> to force better content to the top.
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card style={{ background:"#0F1923", border:"0.5px solid rgba(56,189,248,0.2)" }}>
        <CardContent style={{ padding:"12px 16px" }}>
          ...
          <div style={{ fontSize:11, color:"#94A3B8", lineHeight:1.7 }}>
            <span style={{ color: CAT_COLOR[top[0]] || "#F59E0B", fontWeight:600 }}>{top[0]}</span> content gets the most exposure ({(top[1] as number).toFixed(1)}%) with a quality score of <span style={{ color:"#38BDF8" }}>{topQ}%</span>.{" "}
            <span style={{ color: CAT_COLOR[bottom[0]] || "#818CF8", fontWeight:600 }}>{bottom[0]}</span> gets the least ({(bottom[1] as number).toFixed(1)}%) despite a quality score of <span style={{ color:"#38BDF8" }}>{bottomQ}%</span>.{" "}
