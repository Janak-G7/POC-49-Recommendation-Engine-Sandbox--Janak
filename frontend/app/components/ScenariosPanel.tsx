    ...
    <div className="flex flex-col gap-4">
      ...
      {toast && (
        <div style={{
          ...
          background: toast.startsWith("✓") ? "rgba(52,211,153,0.15)" : toast.startsWith("↺") ? "rgba(56,189,248,0.15)" : "rgba(248,113,113,0.15)",
          border: `0.5px solid ${toast.startsWith("✓") ? "rgba(52,211,153,0.5)" : toast.startsWith("↺") ? "rgba(56,189,248,0.5)" : "rgba(248,113,113,0.5)"}`,
          color: toast.startsWith("✓") ? "#34D399" : toast.startsWith("↺") ? "#38BDF8" : "#F87171",
      ...
      <div style={{ background: "#0F1923", border: "0.5px solid #1F2937", borderRadius: 8, padding: "12px 16px" }}>
        <p style={{ fontSize: 11, color: "#94A3B8", lineHeight: 1.7, margin: 0 }}>
          <span style={{ color: "#38BDF8", fontWeight: 600 }}>Real-world algorithm presets:</span> Every major platform has a different objective. TikTok optimises for watch-time. The BBC optimises for trust. Load any scenario to instantly see how those choices reshape what gets surfaced — and what gets buried. <span style={{ color: "#64748B" }}>Weights update in the Preference Sliders tab automatically.</span>
        </p>
      </div>

      {scenarios.map((s) => {
        const color = SCENARIO_COLORS[s.id] || "#38BDF8";
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginBottom: 12 }}>
                  {Object.entries(s.weights).map(([k, v]) => (
                    <span key={k} style={{
                      ...
                      border: `0.5px solid ${v >= 0.7 ? color + "50" : "#1F2937"}`,
                ...
                <button onClick={() => handleLoad(s)} style={{
                  ...
                  border: `0.5px solid ${st === "failed" ? "rgba(248,113,113,0.5)" : st === "loaded" ? color : "#1F2937"}`,
      ...
      {active && (
        <button onClick={handleReset} style={{
          ...
          border: "0.5px solid #1F2937", borderRadius: 6,
