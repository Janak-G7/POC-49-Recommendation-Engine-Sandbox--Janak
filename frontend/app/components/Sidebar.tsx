...
function StatPill({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ background: "#0B1117", border: "0.5px solid #1F2937", borderRadius: 6, padding: "8px 12px", flex: 1 }}>
...
export default function Sidebar({ avgScore, avgQuality, topCategory, category, onCategoryChange, weights, totalItems }: Props) {
  return (
    <div style={{
      ...
      overflowY: "auto", background: "#030712", borderLeft: "1px solid #1F2937",
    }}>
      ...
      <div style={{ padding: "14px 16px", borderBottom: "1px solid #1F2937" }}>
        ...
        <div style={{ fontSize: 13, fontWeight: 700, color: "#38BDF8", letterSpacing: "-0.02em", marginBottom: 2 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
          <StatPill label="Avg Score">
            <AnimatedNumber value={avgScore} decimals={3} color="#38BDF8" fontSize={17} />
          </StatPill>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <StatPill label="Top Category">
            <div style={{ fontSize: 14, fontWeight: 800, color: "#818CF8" }}>{topCategory || "—"}</div>
          </StatPill>
      ...
      <div style={{ padding: "14px 16px", borderBottom: "1px solid #1F2937" }}>
        ...
        <div style={{ fontSize: 11, color: "#94A3B8", lineHeight: 1.7, marginTop: 8 }}>
          This sandbox makes those signals <span style={{ color: "#38BDF8" }}>inspectable and touchable</span> — turning abstract algorithm talk into something everyday viewers, builders, and allocators can actually understand and challenge.
        </div>
        <div style={{ marginTop: 10, display: "flex", flexWrap: "wrap", gap: 4 }}>
          {[["Content Ranking", "#38BDF8"], ["Feedback Loops", "#818CF8"], ["Popularity Bias", "#F59E0B"], ["Demand Signals", "#34D399"]].map(([label, color]) => (
      ...
      <div style={{ padding: "14px 16px", borderBottom: "1px solid #1F2937" }}>
        ...
        <div style={{ fontSize: 11, color: "#94A3B8", lineHeight: 1.7, marginBottom: 10 }}>
          A handful of engineers at <span style={{ color: "#818CF8" }}>YouTube, TikTok, Netflix, Spotify, and Meta</span> set the objective functions that govern what content surfaces globally. Creators, journalists, and audiences have <strong style={{ color: "#F87171" }}>zero visibility</strong> into these functions — or recourse when they change.
        </div>
        <div style={{ background: "#0B1117", border: "0.5px solid #1F2937", borderRadius: 7, padding: "10px 12px" }}>
          <div style={{ fontSize: 9, color: "#64748B", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 8 }}>
            Data Sources
          </div>
          {[
            { icon: "📡", name: "GDELT", note: "Event-level content signals", tag: "mock", tagColor: "#818CF8" },
            { icon: "🌍", name: "Our World in Data", note: "Consumption baselines", tag: "mock", tagColor: "#818CF8" },
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
          {CATEGORIES.map(c => (
            <button key={c} onClick={() => onCategoryChange(c)} style={{
              ...
              background: category === c ? `${CAT_COLOR[c] || "#38BDF8"}15` : "transparent",
              color: category === c ? (CAT_COLOR[c] || "#38BDF8") : "#64748B",
              border: `0.5px solid ${category === c ? (CAT_COLOR[c] || "#38BDF8") + "50" : "#1F2937"}`,
              transition: "all 0.15s",
            }}>{c}</button>
          ))}
        </div>
        <div style={{ marginTop: 12, padding: "8px 10px", background: "#0B1117", border: "0.5px solid #1F2937", borderRadius: 6, fontSize: 10, color: "#64748B", lineHeight: 1.6 }}>
          💡 <span style={{ color: "#38BDF8" }}>Tip:</span> Load a scenario from the last tab, then come back here to see how the category distribution shifts.
        </div>
      </div>
      ...
      <div style={{ padding: "14px 16px" }}>
        ...
        <button onClick={() => downloadSample(weights)} style={{
          width: "100%", background: "rgba(56,189,248,0.07)", color: "#38BDF8",
          border: "0.5px solid rgba(56,189,248,0.35)", borderRadius: 7,
