...
function D3PopularityChart({ items, counts }: { items: FeedbackItem[], counts: Record<string,number> }) {
  ...
  useEffect(() => {
    ...
    g.selectAll(".bar").data(data).join("rect")
      ...
      .attr("fill", d => d.click_count > 0 ? "#F59E0B" : "#38BDF8").attr("opacity", 0.85)
...
export default function BiasLoop({ items, clicks, popularityGap, gini, totalClicks, onItemClick, onReset }: Props) {
  ...
  const giniOption = {
    ...
    xAxis:{ type:"category", data:["Ideal (Equal)","Current State"], axisLabel:{ color:"#64748B", fontSize:10 }, axisLine:{ lineStyle:{ color:"#1F2937" } } },
    ...
    series:[{ type:"bar", barWidth:40, data:[
      ...
      { value: Math.min(gini,1), itemStyle:{ color: gini>0.5?"#F87171":gini>0.3?"#F59E0B":"#38BDF8", borderRadius:[3,3,0,0] } },
    ]}],
  };
    ...
    <div className="flex flex-col gap-4">
      ...
      <Card style={{ background:"#0F1923", border:"0.5px solid #1F2937" }}>
        ...
        <CardContent style={{ padding:"14px 16px" }}>
            ...
            {items.slice(0,9).map(item => {
              ...
              return (
                <button key={item.id} onClick={() => onItemClick(item.id)} className="rr-card-hover" style={{
                  background: hot ? "rgba(245,158,11,0.07)" : "#0B1117",
                  border:`0.5px solid ${hot ? "rgba(245,158,11,0.45)" : "#1F2937"}`,
                  ...
                  <div style={{ background:"#1F2937", height:3, borderRadius:2 }}>
                    <div style={{ background: hot?"#F59E0B":"#38BDF8", height:3, borderRadius:2, width:`${displayPop*100}%`, transition:"width 0.4s ease" }} />
                  </div>
                  <div style={{ fontSize:9, color:"#64748B", marginTop:3 }}>
                    score: <span style={{ color: hot?"#F59E0B":"#38BDF8", fontWeight:600 }}>{displayScore.toFixed(3)}</span>
                  </div>
                </button>
              );
            })}
          ...
          <div style={{ display:"flex", gap:14, padding:"10px 0", borderTop:"1px solid #1F2937", flexWrap:"wrap", alignItems:"center" }}>
            {[
              { label:"Total Clicks", val:totalClicks, color:"#38BDF8" },
            ...
            <Button variant="ghost" size="sm"
              ...
              style={{ marginLeft:"auto", color:"#64748B", border:"0.5px solid #1F2937", fontSize:10, textTransform:"uppercase" }}>
              Reset
            </Button>
          </div>
        </CardContent>
      ...
      {showComparison && initialTop3.current && (
        <Card style={{ background:"#0F1923", border:"0.5px solid rgba(245,158,11,0.3)", animation:"fadeIn 0.4s ease" }}>
          <CardHeader style={{ padding:"14px 16px 0" }}>
            <div style={{ fontSize:10, color:"#64748B", textTransform:"uppercase", letterSpacing:"0.08em" }}>
            ...
            {LOOP_STAGES.map((stage,i) => (
              <div key={i}>
                <div style={{
                  ...
                  border:`0.5px solid ${i<=activeStage ? "rgba(56,189,248,0.35)" : "#1F2937"}`,
                  borderRadius:6, background: i<=activeStage ? "rgba(56,189,248,0.04)" : "transparent",
                ...
                {i < LOOP_STAGES.length-1 && (
                  <div style={{ fontSize:11, color: i<activeStage ? "#38BDF8":"#1F2937", textAlign:"center", marginBottom:4, transition:"color 0.3s" }}>↓</div>
                )}
              </div>
            ))}
          </CardContent>
